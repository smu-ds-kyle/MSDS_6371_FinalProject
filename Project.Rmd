...
preprocess_data <- function(data_t){
...
preprocess_data_with_response <- function(data_t){

  data_t <- data
  data_clean <- preprocess_data(data_t)
...
test_data <- test_data %>% dplyr::filter(Neighborhood %in% c('NAmes','Edwards','BrkSide'))

predictions <- predict(m4, newdata = test_data)
...
results %>% select(Id, SalePrice) %>% write.csv("predictions/results_analysis_1.csv", row.names = FALSE)
...
Need to check if this is the same mode as above, since the kaggle scores were the same.  or bug in my code. 
...
data_q <- data_all %>% filter(Id %in% c(376,1422, 810, 1182, 599, 956, 198, 1385, 1170, 399))
...
bins <- woebin(data_all %>% where(is.factor), y = "SalePriceLog")
