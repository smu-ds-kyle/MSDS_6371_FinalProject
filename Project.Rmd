...
model <- train(SalePrice ~ GarageArea, data = data_all, method = "lm", trControl = ctrl)
...
col_to_remove <- c('FirstFlrSF','SecondFlrSF','LowQualFinSF','BsmtFinSF1','BsmtFinSF2', 'BsmtUnfSF', 'GarageArea', 'GrLivArea')
data_continuous_clean_t2 <- data_continuous %>% dplyr::select(-all_of(col_to_remove))
base_model_clean_t2<- lm(SalePrice ~., data = data_continuous_clean_t2)
vifs_t2 <- vif(base_model_clean_t2) 
print(vifs_t2)
```
...
Let's take the continuous columns we found in the VIF analysis and then the most significant categorical variables we found. Perhaps we can use LEAPS on this smaller set of options. If not exhaustive, we can do step-wise forward, backward, and both.
...
candidate_data <- cbind(continuous_cols, cata_cols)

model_mat <- model.matrix(~ ., data = candidate_data)[, -1]

set.seed(100)
fit_forward <- regsubsets(x = candidate_data, y = data_all$SalePrice, data = candidate_data, nvmax = 20, nbest = 5, method = 'forward')

set.seed(100)
fit_backward <- regsubsets(x = candidate_data, y = data_all$SalePrice, data = candidate_data, nvmax = 36, nbest = 5, method = 'backward')

set.seed(100)
fit_both <- regsubsets(x = candidate_data, y = data_all$SalePrice, data = candidate_data, nvmax = 36, nbest = 5, method = 'seqrep')

fit_sum <- summary(fit_forward)
...
M1 - Found by MASS package, targeting AIC. Seems somewhat problematic perhaps. Histogram of residuals looks great and the residual scatter plot is decent, but the QQ plot looks mildly concerning as well as the leverage plot. Oddly,
...
M1 <- readRDS('models/potential_model_1.rds')
generate_diagnosticPlots(M1)

terms <- all.vars(terms(M1))

data_M1 <- data_all %>% dplyr::select(all_of(terms))
...
ctrl <- trainControl(method = "LOOCV", allowParallel = TRUE, savePredictions = "all")
model <- train(SalePrice ~ ., data = data_M1, method = "lm", trControl = ctrl)
