...
MASS library can target AIC by doing stepwise feature selection. Using "Both" in this instance.
...
Looking for multi-collinearity with CAR package. Currently errors because we have perfect collinearity in the model. Will try to fix later.

```{r}

library(car)

base_model <- lm(SalePrice ~., data = data_all)

alias(SalePrice ~., data = data_all)
...
Limited Leaps as a full run would take too long.

```{r}

library(leaps)

fit <- regsubsets(SalePrice ~ ., data = data_all, nvmax = 5, nbest = 5, method = 'exhaustive')
